...
class AddInterface(forms.Form):
    ...
    itype = forms.ChoiceField(required=True,choices=(('brideg','bridge'),('ethernet','ethernet')))
    ...
    def clean_ipv6_addr(self):
        ...
        have_symbol = re.match('^[0-9./]+$', ipv6_addr)
        if not have_symbol:
            raise forms.ValidationError(_('The ipv6 must not contain any special characters'))
        elif len(ipv6_addr) > 100:
            raise forms.ValidationError(_('The ipv6 must not exceed 20 characters'))
        return ipv6_addr
    ...
    def clean_name(self):
        ...
        have_symbol = re.match('^[a-zA-Z0-9.]+$',name)
        if not have_symbol:
    ...
    def clean_netdev(self):
        ...
        have_symobl = re.match('^[a-zA-Z0-9.:]+$',netdev)
        if not have_symobl:
            raise forms.ValidationError(_('The interface must not contain any special characters'))
        elif len(netdev) > 10:
            raise forms.ValidationError(_('The interface must not exceed 10 characters'))
