...
class AddNetPool(forms.Form):
    ...
    bridege_name = forms.CharField(max_length=20,required=False)
    ...
    def clean_name(self):
        ...
        if not have_symbol:
            raise forms.ValidationError(_('The pool must not contain any special characters'))
        elif len(name) > 20:
            raise forms.ValidationError(_('The pool must not exceed 20 characters'))
        return name
    ...
    def clean_bridege_name(self):
        bridge_name = self.cleaned_data['bridege_name']
