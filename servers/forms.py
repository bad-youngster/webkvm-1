...
class ComputeAddTcpForm(forms.Form):
    ...
    def clean_hostname(self):
        ...
        have_symbol = re.match('[^a-zA-Z0-9.-] + ',hostname)
        wrong_ip = re.match('^0.|^255.',hostname)
        if have_symbol:
            raise forms.ValidationError(_('Hostname must contain only number, or the domain name separated by "." '))
        elif wrong_ip:
...
class ComputeEditHostForm(forms.Form):
    ...
    class ComputeAddSocketForm(forms.Form):
        name = forms.CharField(error_messages={'required': _('No hoatname has been entered')},
                               max_length=20)
        ...
        def clean_name(self):
            ...
            if have_symbol:
                ...
                raise forms.ValidationError(_('The host name must not excent 20 characeters'))
            try:
                Compute.objects.get(name=name)
            except Compute.DoesNotExist:
                return name
