...
class WvmStorages(WvmConnect):
    ...
    def create_storage_ceph(self,stg_type,name,ceph_pool,ceph_host,ceph_user,secret):
        xml = """
                ...
                <source>
                    <name>%s</name>
                    <host name='%s' prot='6789'/>
                    <auth username='%s' type='ceph'>
                        <secret uuid='%s' />
                    </auth>
                </source>
                </pool>
        """ % (stg_type,name,ceph_pool,ceph_host,ceph_user,secret)
    ...
    def create_storage_netfs(self,stg_type,name,netfs_host,source,source_format,target):
        ...
        """ % (stg_type,name,netfs_host,source,source_format,target)
        self.define_storage(xml,0)
        stg = self.get_storage(name)
        stg.create(0)
        stg.setAutostart(1)
...
class WvmStorage(WvmConnect):
    ...
    def get_allocation(self):
        return util.get_xml_path(self._XMLDesc(0),"/pool/alloction")

    def get_available(self):
        return util.get_xml_path(self._XMLDesc(0),"pool/available")

    def get_capacity(self):
        return util.get_xml_path(self._XMLDesc(0),"/pool/capacity")
